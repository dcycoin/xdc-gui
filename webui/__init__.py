...
from manager.ProcessManager import XDCdManager, WalletRPCManager
...
class BaseWebUI(QMainWindow):
    def __init__(self, html, app, hub, window_size, debug=False):
        ...
        self.icon = self._getQIcon('xdc_icon_64.png')
...
class MainWebUI(BaseWebUI):
    def __init__(self, app, hub, debug):
        ...
        if sys.platform == 'darwin':
            tray_icon = 'xdc_16x16_mac.png'
        elif sys.platform == "win32":
            tray_icon = 'xdc_16x16.png'
        else:
            tray_icon = 'xdc_32x32_ubuntu.png'
...
#         self.view.load(qt_core.QUrl(self.url))
    ...
    def start_deamon(self):
        #start xdcd daemon
        self.xdcd_daemon_manager = XDCdManager(self.app.property("ResPath"), 
                                            self.app_settings.settings['daemon']['log_level'], 
                                            self.app_settings.settings['daemon']['block_sync_size'])
        
        self.xdcd_daemon_manager.start()
    ...
    def run_wallet_rpc(self, wallet_password, log_level=0):
        ...
        while True:
            ...
            xdcd_info = self.daemon_rpc_request.get_info()
            if xdcd_info['status'] == "OK":
    ...
    def _update_daemon_status(self):
        ...
        if xdcd_info['status'] == "OK":
            status = "Connected"
            self.current_height = int(xdcd_info['height'])
            target_height = int(xdcd_info['target_height'])
        ...
        sync_status = "Disconnected" if xdcd_info['status'] != "OK" else "Synchronizing..."
        if xdcd_info['status'] == "OK" and self.current_height == self.target_height:
...
#             print(json.dumps(wallet_info, indent=4))
    ...
    def about(self):
        QMessageBox.about(self, "About", \
            u"%s <br><br>Copyright© 2018 - X DIGITAL CURRENCY (www.blockchain.net.my)" % self.agent)
    ...
    def _handleAboutToQuit(self):
        ...
        if self.wallet_rpc_manager is not None:
            self.wallet_rpc_manager.stop()
        if self.xdcd_daemon_manager is not None:
            self.xdcd_daemon_manager.stop()
